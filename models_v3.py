...
from timm.models.vision_transformer import Mlp, PatchEmbed , _cfg, resize_pos_embed
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        if fuse_attn:
            ...
            x = F.scaled_dot_product_attention(q, k, v, is_causal=False,scale=self.scale, dropout_p=p)
        else:
            q = q * self.scale
...
def gather(x, index):
    B, N, C = x.shape
    index = index.unsqueeze(2).repeat(1, 1, C)
    return torch.gather(x,dim=1,index=index)

class mvit_models(nn.Module):
    """ Vision Transformer with multi-scale inputs.
    
    """
    def __init__(self, img_size=224, num_frames=1, scale_range=(0.5,1), mask_ratio=0.0,
        ...
        super().__init__()
        print("unknown parameters: ", kwargs)
        ...
        self.patch_embed = Patch_layer(strict_img_size=False, dynamic_img_pad=True,
    ...
    def forward_features(self, img):
        ...
        if self.num_frames == 1:
            ...
            scales = torch.linspace(self.scale_range[0], self.scale_range[1], self.num_frames).tolist()
        x_list = []
        for i in range(self.num_frames):
            if scales[i] != 1.0:
                scaled_img = F.interpolate(img, scale_factor=scales[i], mode='bilinear', align_corners=False)
            else:
                scaled_img = img
            x = self.patch_embed(scaled_img)
...
@register_model
def mvit_tiny_patch16_LS(pretrained=False, img_size=224, pretrained_21k = False,   **kwargs):
    model = mvit_models(
...
@register_model
def mvit_small_patch16_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = mvit_models(
    ...
    if pretrained:
        ...
        print("loading pretrained model result: ", msg)

    return model

@register_model
def mvit_medium_patch16_LS(pretrained=False, img_size=224, pretrained_21k = False, **kwargs):
    model = mvit_models(
...
@register_model
def mvit_base_patch16_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = mvit_models(
...
@register_model
def mvit_large_patch16_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = mvit_models(
...
@register_model
def mvit_huge_patch14_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = mvit_models(
...
@register_model
def mvit_huge_patch14_52_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = mvit_models(
...
@register_model
def mvit_huge_patch14_26x2_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = mvit_models(
...
@register_model
def mvit_Giant_48x2_patch14_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = mvit_models(
...
@register_model
def mvit_giant_40x2_patch14_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = mvit_models(
...
@register_model
def mvit_Giant_48_patch14_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = mvit_models(
        img_size = img_size, patch_size=14, embed_dim=1664, depth=48, num_heads=16, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers = Layer_scale_init_Block, **kwargs)
    return model
...
@register_model
def mvit_giant_40_patch14_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = mvit_models(
        img_size = img_size, patch_size=14, embed_dim=1408, depth=40, num_heads=16, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers = Layer_scale_init_Block, **kwargs)
    #model.default_cfg = _cfg()
...
@register_model
def mvit_small_patch16_36_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = mvit_models(
...
@register_model
def mvit_small_patch16_36(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = mvit_models(
        img_size = img_size, patch_size=16, embed_dim=384, depth=36, num_heads=6, mlp_ratio=4, qkv_bias=True,
...
@register_model
def mvit_small_patch16_18x2_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = mvit_models(
...
@register_model
def mvit_small_patch16_18x2(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = mvit_models(
        img_size = img_size, patch_size=16, embed_dim=384, depth=18, num_heads=6, mlp_ratio=4, qkv_bias=True,
...
@register_model
def mvit_base_patch16_18x2_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = mvit_models(
...
@register_model
def mvit_base_patch16_18x2(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = mvit_models(
        img_size = img_size, patch_size=16, embed_dim=768, depth=18, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),block_layers=Block_paralx2, **kwargs)
...
@register_model
def mvit_base_patch16_36x1_LS(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = mvit_models(
...
@register_model
def mvit_base_patch16_36x1(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = mvit_models(
        img_size = img_size, patch_size=16, embed_dim=768, depth=36, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
